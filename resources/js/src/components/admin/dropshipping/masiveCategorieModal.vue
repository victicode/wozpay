              ...
              <q-card-section class="q-pt-none q-pb-none q-px-none w-100" style="height: 100%; overflow: none;">
                ...
                >
                  ...
                  <div class="flex justify-end q-pt-sm  q-px-md">
                    ...
                    <q-btn label="Siguiente"  color="black" @click="createCategorie()" :loading="loading"/>
                  </div>
                </div>
              </q-card-section>
            </div>
  ...
  export default {
    ...
    setup (props, { emit }) {
      ...
      const createCategorie = () => {
        loading.value= true
        categorieStore.massiveCreate({data: dataForm.value})
        .then((response) => {
          if(response.code !== 200 ) throw response

          setTimeout(() => {
            showNotify('positive', 'Categorias creadas con exito')
            hideModal()
            loading.value = false
          }, 1000);
        })
