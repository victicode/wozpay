    ...
    <Transition name="inFade">
      <div v-if="ready">
        <div class="q-pa-md">
          ...
          <div>
            <q-form id="addAccountForm" class="q-gutter-md" @submit="addAccountBank()">
              <div class="row ">
                <div class="col-12 q-my-md">
                  <q-input class="account_bankForm no-display-value q-pb-none" outlined color="positive"
                    v-model="selectedBank.name" disabled label="Entidad Bancaria" autocomplete="off" disable>
                ...
                <div class="col-12 q-my-md">
                  <q-input class="account_bankForm q-pb-none" outlined clearable :clear-icon="'eva-close-outline'"
                    color="positive" v-model="newBankAccount.number" name="id_user" label="Número de cuenta"
                    autocomplete="off" mask="#### #### #### #### #### #### #### #### ####"
                    :rules="rulesForm('number')" />
                </div>
                <div class="col-12 q-my-md">
                  <q-input class="account_bankForm q-pb-none" outlined clearable :clear-icon="'eva-close-outline'"
                    color="positive" v-model="user.name" disable name="id_user" label="Titular de la cuenta"
                    :rules="rulesForm('owner')" autocomplete="off" />
                </div>
                <div class="col-12 q-my-md">
                  <q-select outlined class="account_bankForm q-pb-none" v-model="newBankAccount.type"
                    :options="['Ahorro', 'Corriente']" label="Tipo de cuenta" clearable :rules="rulesForm('type')"
                    :clear-icon="'eva-close-outline'" dropdown-icon="eva-chevron-down-outline" behavior="menu" />
                </div>
                <div class="col-12 q-my-md">
                  <q-input class="account_bankForm q-pb-none" outlined clearable :clear-icon="'eva-close-outline'"
                    color="positive" v-model="user.dni" name="id_user" label="Número de cédula" mask="###.###.###"
                    reverse-fill-mask :rules="rulesForm('ownerDni')" autocomplete="off" disable />
                </div>

                <div class="col-12 q-my-md q-mb-md q-px-md-xl q-pt-md-sm ">
                  <q-btn id="login-form-button" label="Agrega cuenta bancaria" unelevated no-caps type="submit"
                    color="primary q-py-md" class="full-width" :loading="loading">
