...
const { plans } = storeToRefs(stripeStore)
const payPlan = ref({});
const route = useRoute()
const getPlan = () => {
    loading.value = true
    let planCode = route.query.plan.slice(0, -2)
    let planAmountType = route.query.plan.slice(-1) == 1 ? 'annual' : 'monthly'
    let planPayType = plans.value.find(plan => plan.code == planCode)[planAmountType];
    const { id, name, type, code } = plans.value.find(plan => plan.code == planCode);
    payPlan.value = {
        id,
        name,
        type,
        code,
        ...planPayType,
        typePay: route.query.plan.slice(-1)
    }
}
onMounted(async () => {
    getPlan()
