<template>
  <div style="height: 91vh;">
      <Transition name="inFade">
        <div v-if="bankAccounts.length > 0 && ready" class="h-100">
            ...
            <div class="q-pt-md q-pb-none q-px-md q-px-md-md q-mb-md account__info" v-for="(account, key) in bankAccounts" :key="key">
              <div >
                ...
                <div class="flex justify-between items-center q-mt-md account__info--item">
                  ...
                  <div class="text-caption2 text-weight-medium text-grey-6">{{ account.account_owner }}</div>
                </div>
                <div class="flex justify-between items-center q-mt-md account__info--item">
                  <div class="text-weight-medium text-caption2">Tipo de cuenta</div>
                  <div class="text-caption2 text-weight-medium text-grey-6">{{ account.account_type }}</div>
                </div>
                <div class="flex justify-between items-center q-mt-md account__info--item">
              ...
              <div class="flex items-end justify-end q-mt-md q-pb-md">
                <q-btn unelevated class="q-ml-md" size="sm" round color="grey-7" style="color: black" @click="Action(account.id)" icon="eva-edit-2-outline" />
                <q-btn unelevated class="q-ml-md" size="sm" round color="negative" style="color: black" @click="Action(account.id)" icon="eva-trash-2-outline" />
              </div>
            </div>
          </div>
        ...
        <div v-else class="q-pt-md q-px-lg account__container" >
...
export default {
  setup () {
    ...
    return { bankAccounts, ready, router, wozIcons, selectAccountBank, Action  }
  },
}
</script>
