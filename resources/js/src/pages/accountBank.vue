    ...
    <Transition name="inFade">
          ...
          <div class="q-pt-md q-pb-none q-px-md q-px-md-md q-mb-md account__info" v-for="(account, key) in bankAccounts"
            ...
            <div>
              ...
              <div class="flex justify-between items-center q-mt-md account__info--item">
                ...
                <div class="text-caption2 text-weight-medium text-primary">{{ numberFormat(account.account_owner_dni) }}
                </div>
              </div>
            </div>
            <div class="flex items-end justify-end q-mt-md q-pb-md">
              <q-btn unelevated class="q-ml-md" size="sm" round color="grey-7" style="color: black"
                @click="showModal(account.id, 'update')" icon="eva-edit-2-outline" />
              <q-btn unelevated class="q-ml-md" size="sm" round color="negative" style="color: black"
      ...
      <div v-else-if="bankAccounts.length == 0 && ready" class="h-100">
        <div class="h-100 flex flex-center column q-px-md q-pb-xl">
          <div class="text-weight-bold q-pb-md text-h6 text-center">
            No tienes cuentas de banco asociadas
          </div>
          <div class="w-100 flex justify-center">
            <q-btn label="Agrega cuenta bancaria" unelevated no-caps color="primary q-py-sm"
              class="redirect_button q-py-sm q-px-lg" @click="router.push('/bank')" />
          </div>
        </div>
      </div>
      <div v-else class="q-pt-md q-px-lg account__container w-100">
        <div class="q-pt-md q-pb-none q-px-md q-px-md-md q-mb-md account__info loading" v-for="n in 8" :key="n">
          <div>
            <div class="flex justify-between items-center account__info--bank account__info--item q-py-md"
              v-for="j in 5" :key="j">
