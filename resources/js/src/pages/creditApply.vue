  ...
  <div v-if="showNotReady" style="height: 100%;">
    ...
    <div class="apply_section " v-else>
      <div class="q-pb-xl" v-if="!isCurrentLoan && !load">
        <q-stepper v-model="step" ref="stepper" contracted color="primary" animated class="no-header-steper">
                ...
                <q-item class="q-py- q-px-sm">
                  <q-item-section @click="showInputModal(1, 'business')">
                    <div class="flex items-center justify-between">
                      ...
                      <q-item-label caption lines="1" class="text-weight-medium text-body2">{{ readTapes.business ??
                  ...
                  <q-item-section @click="showInputModal(1, 'business_address')">
                    <div class="flex items-center justify-between">
                      ...
                      <q-item-label caption lines="1" class="text-weight-medium text-body2">{{
                        readTapes.business_address ?? 'Agregar' }}</q-item-label>
                    </div>
                  </q-item-section>
                </q-item>
                <q-separator />
                <q-item class="q-py- q-px-sm">
                  <q-item-section @click="showInputModal(1, 'business_phone')">
                    <div class="flex items-center justify-between">
                      ...
                      <q-item-label caption lines="1" class="text-weight-medium text-body2">{{ readTapes.business_phone
                        ?? 'Agregar' }}</q-item-label>
                    </div>
                  </q-item-section>
                </q-item>
                ...
                <q-item class="q-py- q-px-sm">
                  <q-item-section @click="showInputModal(2, 'amount')">
                    <div class="flex items-center justify-between">
                      ...
                      <q-item-label caption lines="1" class="text-weight-medium text-body2">{{ loan.amount ? 'Gs.' +
                        numberFormat(loan.amount) : 'Seleccionar' }}</q-item-label>
                    </div>
                  </q-item-section>
                </q-item>
                <q-separator />
                <q-item class="q-py- q-px-sm">
                  <q-item-section @click="showInputModal(2, 'due_date')">
                    <div class="flex items-center justify-between">
                      ...
                      <q-item-label caption lines="1" class="text-weight-medium text-body2">{{ loan.due_date ?
                        loan.due_date + ' días' : 'Seleccionar' }} </q-item-label>
                    </div>
                  </q-item-section>
                </q-item>
                <q-separator />
                <q-item class="q-py- q-px-sm">
                  <q-item-section>
                    <div class="flex items-center justify-between">
                      ...
                      <q-item-label caption lines="1" class="text-weight-medium text-body2">Gs. {{
                        numberFormat(loan.amountToPay) }}</q-item-label>
                    </div>
                  </q-item-section>
                </q-item>
                <q-separator />
              ...
              <div class="q-px-md">
                <div class="q-mt-lg more_money_requiriments q-pa-sm">
                  ...
                  <div class="q-mt-md q-pl-lg">
                    ...
                    <div class="q-mb-md text-weight-medium text-body2">• Compra un libro digital de Woz Dropshipping
                    </div>
                  </div>
                </div>
              </div>
          ...
          <template v-slot:navigation>
            <q-stepper-navigation class="q-mt-md flex justify-end q-mx-md-xl q-mb-xl q-pb-xl">
              <q-btn v-if="step > 1" color="grey-6" @click="$refs.stepper.previous()" class="w-100 q-pa-sm q-mb-md"
                label="Volver" />
              <q-btn @click=" step == 1 ? $refs.stepper.next() : createApplyLoan()" color="primary"
                class="w-100 q-pa-sm q-mb-xl" :label="step === 2 ? 'Presentar solicitiud' : 'Siguente'"
                :loading="loading">
      ...
      <div class="q-mt-xl q-px-md" v-else-if="isCurrentLoan && !load">
        <div>
          <div class="text-h6 text-center text-weight-bold">
            {{ statusLoan == 1
              ? 'Tu solicitud de prestamo esta siendo verificada por nuestro equipo.'
              : 'Ya tienes un préstamo activo, termina de pagarlo para optar por otro.' }}
          </div>

          <div class="flex justify-center q-mt-lg ">
            <q-btn color="primary" label="volver" class="q-px-md" size="md" @click="router.push('/dashboard')"
              style="width: 50%;" />
          </div>
        </div>
      </div>
      ...
      <div v-if="dialog == 'setValue'">
        <setValueModal :dialog="(dialog == 'setValue')" :input="input" :days="interestRate.interestRate"
          @hiddeModal="hiddeModal" :isRekutu="false" />
      </div>
      <div v-if="haveRekutu">
  ...
  setup() {
    ...
    const validateUser = () => {
      const dontValidate = [
        ...
        'activate_dropshipping',
        'stripe_id',
        'pm_type',
        'pm_last_four',
        'trial_ends_at',
        'created_at', 'card', 'updated_at', 'deleted_at',]
