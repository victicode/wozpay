...
// Validar si el elemento está listo
const isFormValid = computed(() => {
    return isPaymentElementReady.value && !stripeError.value && !stripeStore.getError;
});
...
onMounted(async () => {
    try {
        ...
        const stripeKey = import.meta.env.VITE_STRIPE_PUBLIC_KEY || 'pk_test_tu_llave_publica';
        ...
        // Obtener el setup intent del backend
        ...
        // Configurar la apariencia minimalista
        const appearance = {
            theme: 'stripe',
            variables: {
                colorPrimary: '#1976D2',
        ...
        // Crear instancia de Elements con el client secret
        ...
        // Configurar opciones del Payment Element
        const paymentElementOptions = {
            ...
            paymentMethodTypes: [
                'card',
                'link',
                // Agregar más métodos según tu configuración en Stripe
            ],
            wallets: {
                applePay: 'auto',
                googlePay: 'auto'
...
const handleProcessPayment = async () => {
    if (!isPaymentElementReady.value) {
        $q.notify({
            type: 'negative',
            message: 'El formulario de pago aún no está listo',
            position: 'top',
            timeout: 3000
        });
        return;
    ...
    try {
        const result = await stripeStore.confirmPayment(
            stripe.value,
            elements.value
        );

        if (result.success && result.paymentMethod) {
            // Obtener el plan_id desde la ruta o props
            // Por ahora usamos un valor por defecto, pero deberías obtenerlo del contexto
            const planId = 'price_12345'; // TODO: Obtener del contexto (props, route, etc.)
            ...
            if (finalizeResult.success) {
                ...
                // Redirigir o actualizar estado del usuario
                // router.push('/dashboard');
            } else {
                $q.notify({
                    type: 'negative',
                    message: finalizeResult.error || 'Error al finalizar el registro.',
                    position: 'top',
                    timeout: 4000
                });
            }
